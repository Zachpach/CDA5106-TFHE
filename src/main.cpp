...
static int64_t modInverse(int64_t a, int64_t p) {
    ...
    if (r != 1) throw std::runtime_error("modInverse: no inverse exists");
    return t < 0 ? t + p : t;
}

// Input: secret: integer to protect, N: total number of shares,
//        prime: prime modulus 
// Output: N shares, each a distinct point on f(x) = secret + a·x (mod prime).
...
int64_t shamirReconstruct(const Share& s1, const Share& s2, int64_t prime) {
    ...
    std::cout << "       Lagrange: y1*L1 = " << y1 << " * inv(" << x1 << "-" << x2 << ") = " << term1
              << ",  y2*L2 = " << y2 << " * inv(" << x2 << "-" << x1 << ") = " << term2
              << ",  sum = " << result << "\n";
...
int main() {
    ...
    for (int x : {0, 1}) {
        for (int y : {0, 1}) {
            ...
            std::cout << "    NAND computation:     " << tstr(toTorus(5.0/8.0))
                      << "  -  [hidden_x] " << tstr(encX.hiddenValue)
                      << "  -  [hidden_y] " << tstr(encY.hiddenValue)
                      << "  =  [result] " << tstr(result.hiddenValue) << "\n";
    ...
    std::cout << "\n    End-to-end (Reconstructing secret from shares 1 & 3):\n";
    {
        ...
        for (int testBit : {0, 1}) {
            std::cout << "      Encrypt bit " << testBit << " (original key), decrypt (reconstructed key):\n";
            Ciphertext ct = encrypt(testBit, key, NOISE_STDDEV);
            int got = decrypt(ct, recoKey);
            bool ok = (got == testBit);
            std::cout << "      Bit " << testBit << " -> " << got
                      << (ok ? "  PASS" : "  FAIL") << "\n";
            ok ? multiPass++ : multiFail++;
        }
    }
